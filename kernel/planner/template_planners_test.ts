...
import { ExplorationPlanner, ImplementationPlanner, RefactoringPlanner, selectPlanner } from "./template_planners.ts";
import { isScoutPattern, isRelayPattern, isSwarmPattern, determineExecutionMode } from "./planner.ts";
import { Mode } from "../types.ts";

Deno.test("ExplorationPlanner creates scout-like graph", async () => {
  const planner = new ExplorationPlanner();
  const graph = await planner.plan({ objective: "Explore auth module", workspace: "/tmp" });
...
Deno.test("ImplementationPlanner creates relay-like graph", async () => {
  ...
  const graph = await planner.plan({ objective: "Implement auth", workspace: "/tmp" });
...
Deno.test("RefactoringPlanner requires analysis task", async () => {
  ...
  const graph = await planner.plan({ objective: "Refactor auth", workspace: "/tmp" });

  const validation = await planner.validate(graph);
  assertEquals(validation.valid, true);

  const badGraph = { ...graph, tasks: graph.tasks.filter((t) => !t.title.includes("Analyze")) };
...
Deno.test("isRelayPattern detects sequential graphs", async () => {
  const planner = new ImplementationPlanner();
  const graph = await planner.plan({ objective: "Implement", workspace: "/tmp" });
