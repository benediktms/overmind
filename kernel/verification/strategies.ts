...
export async function executeLspStrategy(
  ...
  for (const file of strategy.files) {
    ...
    const errorCount = fileDiagnostics.filter((d) =>
      d.severity === "error"
    ).length;
    const warningCount =
...
export async function executeBuildStrategy(
  ...
  const result = await bash.run(
    strategy.command,
    strategy.cwd ?? context.workspace,
  );
  ...
  const artifact: EvidenceArtifact = {
    ...
    summary: result.success
      ? `Build succeeded (${result.duration_ms}ms)`
...
export async function executeAgentStrategy(
  ...
  const response = await neuralLink.waitFor(roomId, participantId, timeoutMs, [
    "review_result",
  ]);
  ...
  const artifact: EvidenceArtifact = {
    type: "diagnostic",
    summary: `Agent ${strategy.agentRole}: ${
...
export async function executeCompositeStrategy(
  ...
  return {
    ...
    confidence,
    details: `${strategy.mode} mode: ${
      allPassed ? "all strategies passed" : "some strategies failed"
    }`,
    evidence,
