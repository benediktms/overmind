---
name: drone
description: General-purpose builder that picks up scoped implementation subtasks off a shared epic task list. Full read/write tooling. Use whenever a build-style agent team needs a workhorse to claim and implement subtasks in parallel. Respects the file ownership declared on each task.
model: sonnet
